...
export default function NotificationPopup({ isVisible, onClose, onAccept, elderName, taskType, taskId, location, urgent, message, emergency_severity }: NotificationPopupProps) {
    ...
    return (
        <div className="fixed inset-0 z-[10001] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
            <div className={`bg-white rounded-[2rem] shadow-2xl border border-slate-200 overflow-hidden transform transition-all duration-500 max-w-lg w-full mx-4 ${
                        ...
                        <>
                            ...
                            <div className="flex items-start gap-5 mb-8">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-slate-500 font-bold text-sm uppercase tracking-tight mb-2">Needs assistance with:</p>
                                    <div className={`inline-block px-4 py-2 rounded-xl font-black text-lg ${
                                        urgent ? 'bg-red-600 text-white shadow-lg shadow-red-200' : 'bg-blue-600 text-white shadow-lg shadow-blue-200'
                                    }`}>
                                        {taskType}
                                    </div>
                                </div>
                        ...
                        <>
                            ...
                            <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
                                <div className="bg-slate-50 rounded-3xl p-6 border border-slate-100">
                                    <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Request Information</h5>
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center shadow-sm border border-slate-100">
                                                <MapPin className="w-4 h-4 text-blue-600" />
                                            </div>
                                            <p className="text-slate-700 font-bold text-sm">{location || 'Chennai, Tamil Nadu'}</p>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center shadow-sm border border-slate-100">
                                                <AlertTriangle className={`w-4 h-4 ${urgent ? 'text-red-600' : 'text-amber-600'}`} />
                                            </div>
                                            <p className="text-slate-700 font-bold text-sm">Severity: <span className={urgent ? 'text-red-600' : 'text-amber-600'}>{emergency_severity}</span></p>
                                        </div>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <button
                                        ...
                                        className={`px-6 py-4 ${urgent ? 'bg-red-600' : 'bg-emerald-600'} text-white rounded-2xl font-black transition-all hover:opacity-90 active:scale-95 shadow-xl ${urgent ? 'shadow-red-200' : 'shadow-emerald-200'}`}
                                    >
                                        Accept Request
                                    </button>
                                </div>
