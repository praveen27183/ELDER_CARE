...
import { MapPin, Navigation, Phone, CheckCircle, AlertTriangle, Star, Award, TrendingUp, List, Power } from 'lucide-react';
...
export default function VolunteerDashboard() {
    ...
    // Notification system with 20-second intervals
    useEffect(() => {
        const showRandomNotification = () => {
            try {
                // Only show if:
                // 1. There are requests
                // 2. No notification is already showing
                // 3. The volunteer is ON DUTY
                // 4. The volunteer is NOT currently on a task
                if (processedRequests.length > 0 && !showNotification && isOnDuty && !activeTask) {
                    const randomTask = processedRequests[0]; // Pick the highest priority task
    ...
    }, [processedRequests, isOnDuty, activeTask, showNotification]);
            ...
            {loading ? (
                ...
                <>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center flex-wrap gap-3">
                                                ...
                                                <button
                                                    ...
                                                    className={`flex items-center gap-2.5 px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all duration-300 border-2 ${isOnDuty
                                                        ? 'bg-emerald-600 text-white border-emerald-500 shadow-xl shadow-emerald-100 hover:bg-emerald-700 hover:scale-105'
                                                        : 'bg-white text-slate-400 border-slate-200 hover:border-slate-300 hover:text-slate-600'
                                                        }`}
                                                >
                                                    <Power className={`w-4 h-4 ${isOnDuty ? 'animate-pulse' : ''}`} />
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-100">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-lg font-medium transition-colors ${activeTab === 'feed'
                        ...
                        <div className="p-6">
                            {!isOnDuty ? (
                                <div className="flex flex-col items-center justify-center py-24 px-4 text-center bg-slate-50/50 rounded-3xl border-2 border-dashed border-slate-200">
                                    <motion.div
                                        initial={{ scale: 0.8, opacity: 0 }}
                                        animate={{ scale: 1, opacity: 1 }}
                                        className="w-32 h-32 bg-white rounded-[2.5rem] flex items-center justify-center mb-8 text-slate-300 border-4 border-slate-50 shadow-2xl shadow-slate-200"
                                    >
                                        <Power className="w-14 h-14" />
                                    </motion.div>
                                    <h3 className="text-3xl font-black text-slate-900 tracking-tight mb-3">You are currently Off Duty</h3>
                                    <p className="text-slate-500 max-w-sm mb-10 font-bold text-lg leading-snug">Toggle your status to "On Duty" to start receiving service requests and helping elders in your area.</p>
                                    <button
                                        onClick={() => setIsOnDuty(true)}
                                        className="bg-emerald-600 hover:bg-emerald-700 text-white px-12 py-5 rounded-[2rem] font-black text-xl shadow-2xl shadow-emerald-200 transition-all active:scale-95 flex items-center gap-4 group hover:-translate-y-1"
                                    >
                                        <Power className="w-8 h-8 group-hover:rotate-12 transition-transform" />
                                        ACTIVATE DUTY NOW
                                    </button>
                                </div>
                            ) : activeTab === 'feed' ? (
                                <div className="space-y-4">
                                    ...
                                    {processedRequests && processedRequests.length > 0 ? (
                                        processedRequests.map((task) => (
                                            <div key={task.id} className={`bg-slate-50 p-4 rounded-lg border-l-4 ${task.emergency_severity === 'HIGH' ? 'border-red-500' :
                                                ...
                                                <div className="flex justify-between items-start mb-2">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className={`text-xs px-2 py-1 rounded-full font-medium ${task.emergency_severity === 'HIGH' ? 'bg-red-100 text-red-800' :
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    <button
                                                        onClick={() => handleAccept(task)}
                                                        className={`flex-1 py-2 rounded-lg font-medium text-sm transition-colors ${(task as any).emergency_severity === 'HIGH'
                                            ...
                                            <div className="bg-white border border-slate-200 rounded-lg p-4">
                                                ...
                                                <div className="space-y-2 max-h-60 overflow-y-auto">
                                                    {currentChecklist.map((item) => (
                                                        <div
                                                            key={item.id}
                                                            className={`flex items-center gap-3 p-2 rounded-lg border cursor-pointer transition-colors ${item.completed
                                                        ...
                                                        >
                                                            <input
                                                                ...
                                                                className="w-4 h-4"
                                                            />
                                                            <span className={`flex-1 text-sm ${item.completed ? 'text-slate-500 line-through' : 'text-slate-700'
                                                                }`}>
                                                                {item.item}
                                                            </span>
                                                            <span className={`text-xs px-2 py-1 rounded ${item.priority === 'critical' ? 'bg-red-100 text-red-700' :
                                    ...
                                    <div className="flex-1 bg-white rounded-lg border border-slate-200 p-6">
                                        ...
                                        <div className="flex flex-col gap-3">
                                            ...
                                            <button
                                                ...
                                                className={`w-full py-4 rounded-xl font-bold shadow-lg transition-all ${currentChecklist.every(item => item.completed)
