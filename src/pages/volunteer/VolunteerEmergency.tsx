...
export default function VolunteerEmergency() {
    ...
    const [activeEmergencies, setActiveEmergencies] = useState<ServiceRequest[]>([]);
    
    useEffect(() => {
        RequestService.loadStoredRequests();
        const unsubscribe = RequestService.subscribe((requests) => {
            // Filter for urgent/emergency requests that are still pending
            const urgent = requests.filter(req => req.urgent && req.status === 'pending');
            setActiveEmergencies(urgent);
        });
        return unsubscribe;
    }, []);
    ...
    const handleViewDetails = (emergency: any) => {
        alert(`Emergency Details:\n\nType: ${emergency.taskType}\nElder: ${emergency.elderName}\nLocation: ${emergency.location}\nMessage: ${emergency.message}\nSeverity: ${emergency.emergency_severity || 'HIGH'}`);
    };
    ...
    const handleAccept = (emergency: ServiceRequest) => {
        try {
                ...
                if (emergency.coordinates) {
                    const navigationData = {
                        to: {
                            ...
                            latitude: emergency.coordinates.latitude,
                            longitude: emergency.coordinates.longitude
                        },
                        from: {
                        ...
                        distance: 0,
                        eta: 5,
                        elderName: emergency.elderName,
                        taskType: emergency.taskType,
                        taskId: emergency.id
                    };
                    sessionStorage.setItem('navigationData', JSON.stringify(navigationData));
                }

                // Redirect to dashboard where the active task checklist will appear
                navigate('/volunteer');
            }
        } catch (error) {
                        ...
                        {activeEmergencies.map((emergency) => (
                            <div key={emergency.id} className="border border-red-200 bg-red-50 p-4 rounded-lg">
                                <div className="flex justify-between items-start mb-3">
                                    ...
                                    <span className={`px-2 py-1 rounded-full text-xs font-medium border ${getSeverityColor(emergency.emergency_severity || 'HIGH')}`}>
                                        {(emergency.emergency_severity || 'HIGH').toUpperCase()}
                                    </span>
                                </div>

                                <div className="text-sm text-slate-600 space-y-1 mb-4">
                                    <p className="font-bold text-slate-800">Elder: {emergency.elderName}</p>
                                    <p className="flex items-center gap-2">
                                        <Navigation className="w-4 h-4" />
                                        {emergency.location}
                                    </p>
                                    {emergency.message && (
                                        <p className="bg-white/50 p-2 rounded border border-red-100 italic text-red-800 mt-2 text-xs">
                                            "{emergency.message}"
                                        </p>
                                    )}
                                </div>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button 
                                        ...
                                        className="flex-1 bg-white text-red-600 border border-red-200 py-2 rounded-lg font-medium text-sm hover:bg-red-50 transition-colors"
                                    >
                                        View Details
                                    </button>
                                </div>
                        ...
                        <div key={index} className={`border rounded-lg p-4 ${getContactColor(contact.type)}`}>
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <button 
                                    ...
                                    className="bg-white text-slate-800 px-3 py-1 rounded-lg font-medium text-sm hover:bg-slate-50 transition-colors cursor-pointer"
                                >
