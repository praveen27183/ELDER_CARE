...
// Open Google Maps Navigation with enhanced features
export function openGoogleMapsNavigation(fromLocation?: { latitude: number; longitude: number }, toLocation: NavigationLocation, options?: NavigationOptions): void {
    const { latitude: toLat, longitude: toLng, address } = toLocation;
    
    let url = `https://www.google.com/maps/dir/?api=1`;
    
    // Add origin if provided
    if (fromLocation) {
        url += `&origin=${fromLocation.latitude},${fromLocation.longitude}`;
    }
...
export function calculateDistance(from: { latitude: number; longitude: number }, to: { latitude: number; longitude: number }): number {
    ...
    const a = 
        ...
        Math.cos(from.latitude * Math.PI / 180) * Math.cos(to.latitude * Math.PI / 180) * Math.cos(dLng / 2);
    const c = 2 * Math.atan2(Math.sqrt(a * a));
    const distance = R * c;
    
    return Math.round(distance * 100) / 100; // Return in meters, rounded
}

// Enhanced navigation with distance and ETA
export function openEnhancedNavigation(fromLocation: { latitude: number; longitude: number }, toLocation: NavigationLocation, options?: NavigationOptions): void {
    const distance = calculateDistance(fromLocation, toLocation);
    const eta = calculateETA(distance, options?.mode || 'driving');
    
    // Store navigation data for map component
    ...
    openGoogleMapsNavigation(fromLocation, toLocation, options);
}
